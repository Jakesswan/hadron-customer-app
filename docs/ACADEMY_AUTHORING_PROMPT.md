...
to the Project knowledge, **also upload `academy.js` and `academy-content.js`** so it matches house style and
doesn't duplicate existing courses, then paste the prompt below into the Project's **custom instructions**.
...
The app already has ~30 courses across 10 tracks. I have uploaded `academy.js`
and `academy-content.js` — read them first. Match their house style and tiny-
markdown dialect exactly. Before writing, tell me whether my request is a NEW
course or a DEEPENING of an existing one, and flag any overlap you find.
